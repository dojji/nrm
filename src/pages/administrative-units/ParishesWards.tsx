...
const ParishesWards = () => {
  const { data: entities, isLoading, refetch } = useGetParishWardsQuery();
  ...
  const [createEntity] = useCreateParishWardMutation();
  const [updateEntity] = useUpdateParishWardMutation();
  const [deleteEntity] = useDeleteParishWardMutation();

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingEntity, setEditingEntity] = useState<ParishWard | null>(null);
  const [newEntity, setNewEntity] = useState<Partial<ParishWard>>({});
  ...
  const getFilteredEntities = () => {
    ...
    switch (user?.role) {
      ...
      case "DistrictRegistra":
        // Filter to only show parishes/wards in user's district
        return entities.filter((entity) => {
          const subcountyDivision = subcountyDivisions?.find(
            (sd) => sd.id === entity.subcountyDivisionId
          );
      ...
      case "RegionalCoordinator":
        // Filter parishes/wards that belong to the coordinator's subregion
        return entities.filter((entity) => {
          const subcountyDivision = subcountyDivisions?.find(
            (sd) => sd.id === entity.subcountyDivisionId
          );
    ...
    try {
      ...
      const csvData = entities.map((entity) => {
        ...
        return `${entity.name},${entity.type},${
          subcountyDivision?.name || ""
        },${constituency?.name || ""},${district?.name || ""},${
          subregion?.name || ""
        },${region?.name || ""}`;
      });
      ...
      saveAs(
        blob,
        `parishes-wards_${new Date().toISOString().split("T")[0]}.csv`
      );
            ...
            <div className="space-y-4">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewEntity({
                    ...newEntity,
                    type: e.target.value as "parish" | "ward",
                  })
                }
                className="border p-2 w-full rounded"
              >
              ...
              <select
                value={newEntity.subcountyDivisionId || ""}
                onChange={(e) =>
                  setNewEntity({
                    ...newEntity,
                    subcountyDivisionId: Number(e.target.value),
                  })
                }
                className="border p-2 w-full rounded"
              >
                <option value="">Select Subcounty/Division</option>
                {subcountyDivisions?.map((subcountyDivision) => (
                  <option
                    key={subcountyDivision.id}
                    value={subcountyDivision.id}
                  >
                    {subcountyDivision.name}
                  </option>
                ))}
