...
import { CircleAlert as AlertCircle, CircleCheck as CheckCircle, CreditCard as Edit, Trash } from "lucide-react";
...
const SubcountiesDivisions = () => {
  ...
  const [createEntity] = useCreateSubcountyDivisionMutation();
  const [updateEntity] = useUpdateSubcountyDivisionMutation();
  const [deleteEntity] = useDeleteSubcountyDivisionMutation();

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingEntity, setEditingEntity] = useState<SubcountyDivision | null>(
    null
  );
  const [newEntity, setNewEntity] = useState<Partial<SubcountyDivision>>({});
  ...
  const getFilteredEntities = () => {
    ...
    switch (user?.role) {
      ...
      case "DistrictRegistra":
        // Filter to only show subcounties in user's district
        return entities.filter((entity) => {
          const constituency = constituenciesMunicipalities?.find(
            (c) => c.id === entity.constituencyDivisionId
          );
          return constituency?.districtId === Number(user?.district);
        });

      case "RegionalCoordinator":
        // Filter subcounties that belong to the coordinator's subregion
        return entities.filter((entity) => {
          const constituency = constituenciesMunicipalities?.find(
            (c) => c.id === entity.constituencyDivisionId
          );
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        <div
          ...
          onClick={() => navigate(`/subcounties-divisions/${params.row.id}`)}
        >
          {params.value}
        </div>
      ),
  ...
  const handleExport = () => {
    try {
      ...
      const csvData = entities.map((entity) => {
        ...
        const region = regions?.find((r) => r.id === subregion?.regionId);
        return `${entity.name},${entity.type},${constituency?.name || ""},${
          district?.name || ""
      ...
      saveAs(
        blob,
        `subcounties-divisions_${new Date().toISOString().split("T")[0]}.csv`
      );
            ...
            <div className="space-y-4">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewEntity({
                    ...newEntity,
                    type: e.target.value as "subcounty" | "division",
                  })
                }
                className="border p-2 w-full rounded"
              >
              ...
              <select
                value={newEntity.constituencyDivisionId || ""}
                onChange={(e) =>
                  setNewEntity({
                    ...newEntity,
                    constituencyDivisionId: Number(e.target.value),
                  })
                }
                className="border p-2 w-full rounded"
              >
                <option value="">Select Constituency/Municipality</option>
                {constituenciesMunicipalities?.map((constituency) => (
...
export default SubcountiesDivisions;
